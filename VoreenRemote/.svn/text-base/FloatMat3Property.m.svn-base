...
@implementation FloatMat3Property

- (id)initWithXML:(TBXMLElement *)element {
    self = [super initWithXML:element];

    if (![self parseMat3FromElement:element identifier:@"value" target:&_value])
        return nil;
    
    NSMutableDictionary* state = [NSMutableDictionary dictionaryWithCapacity:2];
    if (![self parseMat3FromElement:element identifier:minValue target:&state])
        return nil;
    if (![self parseMat3FromElement:element identifier:maxValue target:&state])
        return nil;
    self.state = state;
    return self;
}
...
- (NSString*)pythonString {
    return [NSString stringWithFormat:@"((%f,%f,%f),(%f,%f,%f),(%f,%f,%f))",
            [self currentValueForRow:0 column:0], [self currentValueForRow:0 column:1], [self currentValueForRow:0 column:2],
            [self currentValueForRow:1 column:0], [self currentValueForRow:1 column:1], [self currentValueForRow:1 column:2],
            [self currentValueForRow:2 column:0], [self currentValueForRow:2 column:1], [self currentValueForRow:2 column:2]];
}
